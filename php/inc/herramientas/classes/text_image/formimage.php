...
include_once ("../framemaker/framemaker.php");
...
$tipo_idioma_traducir=explode('_',$id_traduccion);

if ($tipo_idioma_traducir[0]=='i') {

	$result=$query->buscar_traduccion_por_id($tipo_idioma_traducir[1]);
	$d_palabra=mysql_fetch_array($result);
	$translation=$d_palabra['traduccion']; 
	
	$texto_idioma_principal=$_POST['txt_cas'];

	$res_ar = $utfConverter->strToUtf8($translation);
	$res_ru = $utfConverter_ru->strToUtf8($translation);
	$res_bulg = $utfConverter_ru->strToUtf8($translation);
	
	switch ($d_palabra['idioma']) {
	
		case 'Chino':
		$lang_ext_minusc=$translation; 
		$lang_ext_mayusc=$translation;
		$font_ext="../../../../fonts/Cyberbit.ttf";	
		break;
		
		case 'Rumano':
		$lang_ext_mayusc=strtoupper($translation);
		$lang_ext_minusc=$translation; 
		$font_ext="../../../../plugins/html2ps/fonts/arial.ttf";
		break;
		
		case 'Polaco':
		$lang_ext_mayusc=strtoupper_utf8($translation);
		$lang_ext_minusc=$translation;
		$font_ext="../../../../plugins/html2ps/fonts/arial.ttf";
		break;
...
} elseif ($tipo_idioma_traducir[0]=='c') { // Si el idioma es castellano

	$d_palabra=$query->datos_palabra($tipo_idioma_traducir[1]);
	$translation=$d_palabra['palabra'];
	$texto_idioma_principal=$_POST['txt_cas'];
	$lang_ext_mayusc=strtoupper_utf8($translation);
	$lang_ext_minusc=$translation; 
	$font_ext="../../../../plugins/html2ps/fonts/arial.ttf";
	...
	if ($fuente_castellano=='FreeFarsi') {
		$texto_idioma_principal=fagd($texto_idioma_principal,'','normal');
	}
...
$texto_idioma_principal_mayusc=strtoupper_utf8($texto_idioma_principal); 
$texto_idioma_principal_minusc=$texto_idioma_principal; 
...
switch ($orden) {
	...
	if ($_POST['sup_may']==1) { $texto1=$texto_idioma_principal_mayusc; } else { $texto1=$texto_idioma_principal_minusc; }
	if ($_POST['inf_may']==1) { $texto2=$texto_idioma_principal_mayusc; } else { $texto2=$texto_idioma_principal_minusc; }
	break;
...
if (isset($_REQUEST['di'])) { $di=$_REQUEST['di']; } else { $di=''; }
...
if($fm == "png"){
	...
	echo '</div><br><div align="center"><img src=\'../../../temp/'.$nombre_img.'.png\'><form id="img_subida" name="img_subida" method="post\ action=""><input name="imagen_subida" type="hidden" id="imagen_subida" value="'.$imagen_original.'"/><input name="imagen_actual" type="hidden" id="imagen_actual" value="'.$nombre_img.'.png"/></form></div>';
}
...
if($fm == "jpg"){
	...
	echo '</div><br><div align="center"><img src=\'../../../temp/'.$nombre_img.'.jpg\'><form id="img_subida" name="img_subida" method="post\ action=""><input name="imagen_subida" type="hidden" id="imagen_subida" value="'.$imagen_original.'"/><input name="imagen_actual" type="hidden" id="imagen_actual" value="'.$nombre_img.'.jpg"/></form></div>';
}
...
if($fm == "gif"){
	...
	echo '</div><br><div align="center"><img src=\'../../../temp/'.$nombre_img.'.gif\'><form id="img_subida" name="img_subida" method="post\ action=""><input name="imagen_subida" type="hidden" id="imagen_subida" value="'.$imagen_original.'"/><input name="imagen_actual" type="hidden" id="imagen_actual" value="'.$nombre_img.'.gif"/></form></div>';
