...
<body>
<form action="generar_tableros_rtf.php" method="post" enctype="multipart/form-data" name="generador_paneles" id="generador_paneles">
<div class="body_content" style="height:2200px;">
      <h4><?php echo strtoupper($translate['generador_tableros']); ?>:
      <div style="float:right; font-size:0.8em;"><?php if ($_SESSION['language']=='es') { echo '<a href="../../../zona_descargas/documentacion/manual_generador_tableros_es.pdf" target="_blank" title="'.$translate['descargar_manual_pdf'].'">'.$translate['manual'].'</a>'; } elseif ($_SESSION['language']=='fr') { echo '<a href="../../../zona_descargas/documentacion/createur_de_tableaux.pdf" target="_blank" title="'.$translate['descargar_manual_pdf'].'">'.$translate['manual'].'</a>'; } elseif ($_SESSION['language']=='br') { echo '<a href="../../../zona_descargas/documentacion/manual_ferramenta_gerador_prancha.pdf" target="_blank" title="'.$translate['descargar_manual_pdf'].'">'.$translate['manual'].'</a>'; } elseif ($_SESSION['language']=='pt') { echo '<a href="../../../zona_descargas/documentacion/manual_ferramenta_gerador_painéis_pt.pdf" target="_blank" title="'.$translate['descargar_manual_pdf'].'">'.$translate['manual'].'</a>'; } else { echo '<a href="../../../zona_descargas/documentacion/boards_generator_tool_manual.pdf" target="_blank" title="'.$translate['descargar_manual_pdf'].'">'.$translate['manual'].'</a>';  }?></div></h4>
	<div id="principal">
    ...
    <div style="width:100%;">
     <fieldset>
       ...
       <p>
         <label><?php echo $translate['filas']; ?>:
           <input name="rows" type="text" id="rows" size="3" maxlength="2" />
         </label>
         <label><?php echo $translate['columnas']; ?>:
       ...
       <p>
         <input type="button" name="crear" id="crear" value="<?php echo $translate['generar_plantilla_previa']; ?>" onclick="procesar('mostrar_panel_plantilla.php','rows='+document.generador_paneles.rows.value+'&cols='+document.generador_paneles.cols.value+'&con_cabecera='+document.generador_paneles.encabezado.value+'&color_borde_tabla='+document.generador_paneles.color_borde_tabla.value+'&ancho_borde_tabla='+document.generador_paneles.ancho_borde_tabla.value+'&tipo_borde_tabla='+document.generador_paneles.tipo_borde_tabla.value+'&img_size_no_text='+document.generador_paneles.tist4.value+'&img_size_with_text='+document.generador_paneles.tict4.value+'&default_posicion_texto_celda='+document.generador_paneles.posic_texto_celda4.value+'','b1');" style="font-size:1.2em;" />
         <br />
       </p>
     </fieldset>
     <fieldset>
...
<br /><?php echo $translate['margenes']; ?> (<?php echo $translate['en_cm']; ?>): <?php echo $translate['superior']; ?>
       <label>
         <input name="margen_superior" type="text" id="margen_superior" value="1" size="1" maxlength="3" />
       </label>
<?php echo $translate['inferior']; ?>
<input name="margen_inferior" type="text" id="margen_inferior" value="2" size="1" maxlength="3" />
<?php echo $translate['derecho']; ?>
<input name="margen_derecho" type="text" id="margen_derecho" value="3" size="1" maxlength="3" />
<?php echo $translate['izquierdo']; ?>
<input name="margen_izquierdo" type="text" id="margen_izquierdo" value="3" size="1" maxlength="3" />
(<?php echo $translate['decimales_separados_punto']; ?>)<br />
	<?php echo $translate['altura_celdas']; ?>: 
       <label>
         <select name="ajuste_altura_celdas" id="ajuste_altura_celdas">
           		<option value="1" selected="selected"><?php echo $translate['automática']; ?></option>
           		<option value="0"><?php echo $translate['manual']; ?></option>
           </select>
        </label>
         <input name="altura_celdas" type="text" id="altura_celdas" size="1" maxlength="4" />
<?php echo $translate['en_cm']; ?> (<?php echo $translate['decimales_separados_punto']; ?>)    
       <input name="n_paneles" type="hidden" id="n_paneles" value="1" />
       <input name="aplicar_configuracion_a" type="hidden" id="aplicar_configuracion_a" value="4" />
       <br />
       <?php echo $translate['anchura_celdas']; ?>: 
     <select name="ajuste_anchura_celdas" id="ajuste_anchura_celdas">
       <option value="1" selected="selected"><?php echo $translate['automática']; ?></option>
       <option value="0"><?php echo $translate['manual']; ?></option>
     </select>
     <input name="anchura_celdas" type="text" id="anchura_celdas" size="1" maxlength="4" />
<?php echo $translate['en_cm']; ?> (<?php echo $translate['decimales_separados_punto']; ?>)<br />
     </p>
     </fieldset>  
     <fieldset>
     ...
     <div id="4" style="display:block;"> <!-- ABRO LA CAPA 4 -->
       ...
       <input type="button" value="<?php echo $translate['aplicar_configuracion_todas_celdas']; ?>" onclick="configurar_todas_celdas_tableros(
