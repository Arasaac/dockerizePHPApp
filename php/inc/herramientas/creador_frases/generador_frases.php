...
//require("../../../classes/graficas/jpgraph-3.5.0b1/jpgraph.php");
//require("../../../classes/graficas/jpgraph-3.5.0b1/jpgraph_scatter.php");
...
if (isset($_POST['frase_secciones'])) $frase_secciones=$_POST['frase_secciones'];
if (isset($_POST['frase'])) $frase1=$_POST['frase'];
...
if (isset($_POST['color_contraste'])) { $color_contraste=$_POST['color_contraste']; } else { $color_contraste=''; };
...
foreach ($_POST['thelist2'] as $indice=>$valor){ 
	...
	if ($v[1] ==1) { //Si el valor es numero es que tengo que añadir una imagen
		...
		if ($simbolo==1) {
			...
			if ($posic_frase==3) { 
				...
				if ($may_frase==1) {
					$palabra_inferior=strtoupper_utf8($palabra_seccion[$n_elementos]);
				} else {
					$palabra_inferior=$palabra_seccion[$n_elementos];
				}
				
				$imagen_fila_txt=array();
				$imagen_fila_txt[]=crear_pictograma($imagen_original,$accion,$fuente_simbolo,$pixels_lienzo_simbolo,$pixels_simbolo_final,$grosor_borde_simbolo,$color_borde,$color_contraste,$id_traduccion,$palabra_seccion[$n_elementos],$sup_idioma,$inf_idioma,$sup_may,$inf_may,$sz_f_s,$color_sup,$color_inf,$sz_f_i,$marco_simbolo);
				$imagen_fila_txt[]=generar_simbolos_solo_texto($pixels_simbolo_final,150,$size_font_frase,$color_frase,$fuente_frase,'center',$palabra_inferior,$salida);
				$img_para_procesar[]='../../../temp/'.encadenar_imagenes(1,$imagen_fila_txt,$salida);
				
			} elseif ($posic_frase==4) { 
	...
	} elseif ($v[1] ==2) { // Si el valor no es número es que tengo que generar un pictograma con la palabra
	
		if ($pictos_may==1) {
			$texto=strtoupper_utf8($v[0]);
		} else { $texto=$v[0]; }
			...
			if ($simbolo==1) {
				...
				if ($posic_frase==3) { 
					...
					$imagen_fila_txt2[]=crear_pictograma($pictograma_generado,$accion,$fuente_simbolo,$pixels_lienzo_simbolo,500,$grosor_borde_simbolo,$color_borde_simbolo,$color_contraste,$id_traduccion,$palabra_seccion[$n_elementos],$sup_idioma,$inf_idioma,$sup_may,$inf_may,$sz_f_s,$color_sup,$color_inf,$sz_f_i,$marco_simbolo);
					$imagen_fila_txt2[]=generar_simbolos_solo_texto(500,150,$size_font_frase,$color_frase,$fuente_frase,'center',$palabra_inferior,$salida);
					$img_para_procesar[]='../../../temp/'.encadenar_imagenes(1,$imagen_fila_txt2,$salida);
...
if ($posic_frase > 0 && $posic_frase < 3) {
...
// **************************************************************************************************
	...
	echo '<br/><br/><div align="center"><img src='.$new_name.'></div><br/><br/><br/><br/><br/><br/>';
	
	echo '<input type="hidden" id="GO" name="GO" size="30" value="http://catedu.es/arasaac/inc/herramientas/creador_frases/imprimir_frase.php?img='.$frase_inicial.'"/>';
?>
