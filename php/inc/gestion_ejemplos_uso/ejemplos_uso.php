...
if (!isset($_POST['pg'])) {
	...
	$contar=$query->ultimas_fichas_eu();
	$resultados=$query->ultimas_fichas_eu_limit($inicial,$cantidad);
	
	$edicion="<a href=\"plugins/javapostlet/index_ejemplos_uso.php\" onclick=\"return GB_showCenter('SUBIR ARCHIVOS EJEMPLOS USO', this.href, 550, 780)\"><img src=\"images/page_white_get.png\" alt=\"Subir archivos\" border=\"0\" /></a> &nbsp;<a href=\"inc/gestion_ejemplos_uso/gestionar_ejemplos_uso.php?i=<?php echo rand(1000000,9999999); ?>\" onclick=\"return GB_showCenter('Añadir nueva ficha de Ejemplos de Uso', this.href, 550, 1024)\"><img src=\"images/page_white_add.png\" alt=\"Añadir nueva ficha de Ejemplos de Uso\" border=\"0\" /></a>";

	$total_records = $contar;
	$total_pages = intval($total_records / $cantidad);
?>
<h4><?php echo $translate['ejemplos_de_uso']." ". $edicion; ?> 
...
<div class="material" style="padding:10px; border:1px solid #CCCCCC; margin-bottom:15px;">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <td width="35%" height="45" valign="top"><b><?php echo $row['eu_titulo']; ?></b>
          <?php 
      ...
      <td width="15%" align="right" valign="top"><?php 	if (isset($_SESSION['AUTHORIZED']) && $_SESSION['AUTHORIZED']== true) { echo '<a href="inc/gestion_ejemplos_uso/gestionar_ejemplos_uso.php?id='.$row['id_eu'].'&i='.rand(1000000,9999999).'" onclick="return GB_showCenter(\'Editar ficha Ejemplo de Uso\', this.href, 550, 1024)"><img src="images/page_edit.png" alt="Editar ficha Ejemplo de Uso" title="Editar ficha Ejemplo de Uso" border="0" /></a>&nbsp;';
	  
				if ($row['eu_estado']==0) { echo '<img src="images/no_visible.gif" alt="Ficha no visible" title="Ficha no visible"border="0">';  }
				elseif ($row['eu_estado']==1) { echo '<img src="images/visible.gif" alt="Ficha visible" title="Ficha visible" border="0">'; }
				elseif ($row['eu_estado']==2) { echo '<img src="images/pendiente_revision.gif" alt="Ficha pendiente de revisi&oacute;n" title="Ficha pendiente de revisi&oacute;n" border="0">'; 	}
...
if ($row['logo_licencia'] != '') { echo '&nbsp;<a href="'.$row['link_licencia'].'" target="_blank"><img src="images/'.$row['logo_licencia'].'" alt="'.utf8_encode($row['licencia']).'" title="'.utf8_encode($row['licencia']).'" border="0" /></a>';  } else {  echo utf8_encode($row['licencia']); } ?>      </td>
    ...
    <tr>
      <td colspan="5" valign="top"><?php echo $row['eu_descripcion']; ?></td>
    </tr>
    <tr>
      <td height="19" colspan="4"><?php 
		  ...
		  $ma=explode(',',$ma); 
			  ...
			  for ($i=0;$i<count($ma);$i++) { 
				if ($ma[$i]!='') {
				 echo '<a href="zona_descargas/ejemplos_uso/'.$row['id_eu'].'/'.$ma[$i].'" target="_blank"><img src=\'images/download1.png\' border="0" alt="Descargar archivo" title="Descargar archivo"><a/><a href="zona_descargas/ejemplos_uso/'.$row['id_eu'].'/'.$ma[$i].'" target="_blank">'.$ma[$i].'</a><br />'; 
				}
			  }
...
<div align="center" class="textos"><strong><?php echo $translate['material']; ?>: </strong><?php echo $inicial ?> <?php echo $translate['a']; ?> <?php echo $inicial+$cantidad ?> <?php echo $translate['de']; ?> <?php echo $total_records ?></div> 
	<div align="center">
	  <?php 
        ...
        $content.= "<a href=\"javascript:void();\" onclick=\"cargar_div('inc/gestion_ejemplos_uso/ejemplos_uso.php','pg=0','principal');\"><< </a>&nbsp;";
        ...
        $prev = ($pg - 1);
        $content.= "<a href=\"javascript:void();\" onclick=\"cargar_div('inc/gestion_ejemplos_uso/ejemplos_uso.php','pg=".$prev."','principal');\"> <</a>&nbsp;";
        ...
        if($total_pages >= $limitpages) {
            ...
            for($i = $page_start-$limite_inferior; ($i <= $total_pages & $i <=$page_limit); $i++) {
                ...
                if(($i) >= 0) { 	
                    ...
                    $content.= "<a href=\"javascript:void();\" onclick=\"cargar_div('inc/gestion_ejemplos_uso/ejemplos_uso.php','pg=".$i."','principal');\">".$i."</a>&nbsp;";
                    }
                }
        ...
        $next = ($pg + 1);
        $content.= "<a href=\"javascript:void();\" onclick=\"cargar_div('inc/gestion_ejemplos_uso/ejemplos_uso.php','pg=".$next."','principal');\"> ></a>&nbsp;";
        ...
        $last = $total_pages;
        $content.= "<a href=\"javascript:void();\" onclick=\"cargar_div('inc/gestion_ejemplos_uso/ejemplos_uso.php','pg=".$last."','principal');\">  >></a>&nbsp;";
