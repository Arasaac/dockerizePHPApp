...
for ($idm=0;$idm<10;$idm++) {
	...
	switch ($idm) {
		...
		if ($hay_traduccion > 0) { 
			...
			} // Cierro el While que recorre las traducciones
		} // Cierro el IF de comprobar si hay traducciones	
		break;
	...
	/* Si hay más de una traducción para la palabra y el idioma concreto....................  */
	
	for($y = 0; $y < count($contenido_traduccion); $y++){ 

	//Si el contenido no es NULL (esta sin traducir) no ejecuto el código y no genero el simbolo
	if ($contenido_traduccion[$y] != NULL) { 
	...
	$nombre_img=$num_caracteres_castellano.'_'.$num_caracteres_traduccion.'_'.$row['id_palabra'].'_'.basename(tempnam("../../temp/",'tmp')); 
		...
		switch ($i) {
			...
			case 1: //Simbolo normal con texto en castellano y minusculas abajo. Con Marco
				...
				$inf_font=1;
				$sup_id_traduccion=$id_traduccion[$y];
				$inf_id_traduccion=0;
				...
				$color_sup="#000000";
				$color_inf="#000000";
				$sz=$idioma_size_letter[$y]; //tamaño letra superior
			...
			case 13: //Simbolo normal con texto en castellano y minusculas abajo. Con Marco
				...
				$sup_id_traduccion=0;
				$inf_id_traduccion=$id_traduccion[$y];
				
				$color_sup="#000000";
				$color_inf="#000000";
				$sz=$idioma_size_letter[$y]; //tamaño letra superior
				$sz1=$idioma_size_letter[$y]; //tamaño letra inferior
				$marco=1;
				$pixels_borde=15;
	...
	$existe_tmp=$query->buscar_si_existe_simbolo_tmp($id_palabra,$id_tipo_simbolo,$marco,$contraste,$sup_con_texto,$superior_idioma,$sup_mayusculas,$sup_font,$inf_con_texto,$inferior_idioma,$inf_mayusculas,$inf_font,$id_imagen,$sup_id_traduccion,$inf_id_traduccion);
	
	$existe_final=$query->buscar_si_existe_simbolo_final($id_palabra,$id_tipo_simbolo,$marco,$contraste,$sup_con_texto,$superior_idioma,$sup_mayusculas,$sup_font,$inf_con_texto,$inferior_idioma,$inf_mayusculas,$inf_font,$id_imagen,$sup_id_traduccion,$inf_id_traduccion);
	...
	switch ($d_palabra['idioma']) {
		...
		case 'Chino':
		$lang_ext_minusc=$contenido_traduccion[$y]; 
		$lang_ext_mayusc=$contenido_traduccion[$y];
		$font_ext="../../fonts/Cyberbit.ttf";	
		break;
		
		case 'Rumano':
		$lang_ext_mayusc=strtoupper($contenido_traduccion[$y]);
		$lang_ext_minusc=$contenido_traduccion[$y]; 
		$font_ext="../../plugins/html2ps/fonts/arial.ttf";
		break;
		
		case 'Polaco':
		$lang_ext_mayusc=strtoupper_utf8($contenido_traduccion[$y]);
		$lang_ext_minusc=$contenido_traduccion[$y];
		$font_ext="../../plugins/html2ps/fonts/arial.ttf";
		break;
		
		case 'Ruso':
		$res_ru = $utfConverter_ru->strToUtf8($contenido_traduccion[$y]);
		...
		case 'Bulgaro':
		$res_bulg = $utfConverter_ru->strToUtf8($contenido_traduccion[$y]);
		...
		case 'Arabe':
		$res_ar = $utfConverter->strToUtf8($contenido_traduccion[$y]);
	...
	$simbolo_tmp=$query->grabar_simbolo_tmp($id_palabra,$id_tipo_simbolo,$marco,$contraste,$sup_con_texto,$superior_idioma,$sup_mayusculas,$sup_font,$inf_con_texto,$inferior_idioma,$inf_mayusculas,$inf_font,$imagen_tratada,$sup_font_size,$sup_font_color,$inf_font_size,$inf_font_color,$id_imagen,$sup_id_traduccion,$inf_id_traduccion);
	...
	//***********************************************************
		...
		//***********************************************************
			...
			//***********************************************************
			} //Cierro el IF que comprueba que la traduccion no es NULL "sin traduccion"
		//***********************************************************
		}// Cierro el For que crea simbolos por cada traduccion que haya de un mimos idioma
	//***********************************************************
