...
$query=new query();
$translate=$query->get_internacionalizacion_page_content($_SESSION['language'],9); 
...
?>
<h4><?php echo $translate['catalogo_de_imagenes']; ?>:</h4>
<div style="width:100%">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="5%" align="center" valign="middle"><a class="grey" href="rss/subscripcion.php?t=4&id_tipo=2" target="_blank"><img src="images/rss2.png" alt="<?php echo $translate['subscribirse_este_catalogo']; ?>" title="<?php echo $translate['subscribirse_este_catalogo']; ?>" border="0" /></a></td>
          <td width="74%" align="left"><?php echo $translate['explicacion_catalogo_imagenes']; ?> </td>
          <td width="8%" align="center" valign="bottom" style="font-size:9px;"><a class="grey" href="javascript:void(0);" onclick="Effect.BlindDown('avanzada_imagenes');; return false;"><img src="images/kappfinder.gif" alt="<?php echo $translate['busqueda_avanzada']; ?>" title="<?php echo $translate['busqueda_avanzada']; ?>" border="0" /></a><br/>
          ...
          <td width="8%" align="center" valign="bottom" style="font-size:9px;"><a href="javascript:void(0);" onclick="treemenu.start(); Effect.BlindDown('derecha');; return false;"><img src="images/view_tree.gif" alt="<?php echo $translate['mostrar_arbol_categorias']; ?>" border="0" title="<?php echo $translate['mostrar_arbol_categorias']; ?>" /></a><br/>
...
<table width="100%" height="440" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td height="259" valign="top">			
      ...
      <div id="avanzada_imagenes" style="font-size: 10px; <?php echo $mostrar_avanzada; ?> margin-bottom:10px; margin-top:5px; float:left; padding-left:5px; border:1px solid #CCCCCC; width: 95%;">
	    <div style="text-align:right; margin-bottom:-25px;"><a href="javascript:void(0);" onclick="Effect.BlindUp('avanzada_imagenes');; return false;">
        <img src="images/close.gif" alt="<?php echo $translate['cerrar_busqueda_avanzada']; ?>"  title="<?php echo $translate['cerrar_busqueda_avanzada']; ?>"  border="0"/></a></div>
<form action="" method="post" name="busqueda_avanzada" id="busqueda_avanzada">
                    ...
                    <tr valign="middle"><br />
                      <td><strong><?php echo $translate['tipo_de_palabra']; ?>:
                        ...
                        <input type="button" name="Submit2" value="<?php echo $translate['buscar']; ?>" onClick="cargar_div('inc/public/ultimas_imagenes.php','id_tipo='+document.busqueda_avanzada.tipo_palabra.value+'&letra='+document.busqueda_avanzada.letra.value+'&filtrado='+document.busqueda_avanzada.filtrado.value+'&orden='+document.busqueda_avanzada.orden.value+'&avanzada=1','principal');" />
                        <br /><br /></td>
                    </tr>
              </table>
		</form>
...
<?php  
				...
				if ($_SESSION['id_language'] > 0) {
					...
					$contar=$query->listar_imagenes($_SESSION['AUTHORIZED'],$id_tipo,$letra,$filtrado,$orden,$id_subtema);
					$resultados=$query->listar_imagenes_limit($_SESSION['AUTHORIZED'],$inicial,$cantidad,$id_tipo,$letra,$filtrado,$orden,$id_subtema);
			...
			?>
          <li> <a href="javascript:void(0);" onclick="Dialog.alert({url: 'inc/public/imagen.php?i=<?php echo $ruta; ?>', options: {method: 'get'}},{windowParameters: {className: 'alphacube', width:600, height:570, hideEffect: Effect.SwitchOff, resizable: true}, okLabel: '<?php echo $translate['cerrar']; ?>'});"><img src="classes/img/thumbnail.php?i=<?php echo $ruta_img; ?>" alt="Image" border="0" class="image" title="<?php echo $word; ?>: <?php if (strlen(utf8_encode($definition)) > 100) { echo substr (utf8_encode($definition), 0, 100)."..."; } else { echo utf8_encode($definition); } ?>" /></a>
...
<td rowspan="2" valign="top">
    ...
    <div style="float:right;"><a href="javascript:void(0);" onclick="Effect.BlindUp('derecha');; return false;"><img src="images/close.gif" alt="<?php echo $translate['cerrar_arbol_categorias']; ?>" title="<?php echo $translate['cerrar_arbol_categorias']; ?>" border="0"/></a></div><br /><br />
    <br /><br />
    <?php
  ...
  <tr>
    <td valign="top">
	<div align="center" class="textos"><strong><?php echo $translate['imagen']; ?>: </strong><?php echo $inicial ?>&nbsp;<?php echo $translate['a']; ?>&nbsp;<?php echo $inicial+$cantidad ?>&nbsp;<?php echo $translate['de']; ?>&nbsp;<?php echo $total_records ?></div> 
	<div align="center">
	  <?php 
        ...
        $content.= "<a href=\"javascript:void(0);\" onclick=\"cargar_div('inc/public/ultimas_imagenes.php','pg=0&id_tipo=".$id_tipo."&letra=".$letra."&filtrado=".$filtrado."&orden=".$orden."&avanzada=".$_POST['avanzada']."&id_subtema=".$id_subtema."','principal');\"><< </a>&nbsp;";
        ...
        $prev = ($pg - 1);
        $content.= "<a href=\"javascript:void(0);\" onclick=\"cargar_div('inc/public/ultimas_imagenes.php','pg=".$prev."&id_tipo=".$id_tipo."&letra=".$letra."&filtrado=".$filtrado."&orden=".$orden."&avanzada=".$_POST['avanzada']."&id_subtema=".$id_subtema."','principal');\"> <</a>&nbsp;";
        ...
        if($total_pages >= $limitpages) {
            ...
            for($i = $page_start-$limite_inferior; ($i <= $total_pages & $i <=$page_limit); $i++) {
                ...
                if(($i) >= 0) { 	
                    ...
                    $content.= "<a href=\"javascript:void(0);\" onclick=\"cargar_div('inc/public/ultimas_imagenes.php','pg=".$i."&id_tipo=".$id_tipo."&letra=".$letra."&filtrado=".$filtrado."&orden=".$orden."&avanzada=".$_POST['avanzada']."&id_subtema=".$id_subtema."','principal');\">".$i."</a>&nbsp;";
                    }
                }
        ...
        $next = ($pg + 1);
        $content.= "<a href=\"javascript:void(0);\" onclick=\"cargar_div('inc/public/ultimas_imagenes.php','pg=".$next."&id_tipo=".$id_tipo."&letra=".$letra."&filtrado=".$filtrado."&orden=".$orden."&avanzada=".$_POST['avanzada']."&id_subtema=".$id_subtema."','principal');\"> ></a>&nbsp;";
        ...
        $last = $total_pages;
        $content.= "<a href=\"javascript:void(0);\" onclick=\"cargar_div('inc/public/ultimas_imagenes.php','pg=".$last."&id_tipo=".$id_tipo."&letra=".$letra."&filtrado=".$filtrado."&orden=".$orden."&avanzada=".$_POST['avanzada']."&id_subtema=".$id_subtema."','principal');\">  >></a>&nbsp;";
        ...
        <div align="center">
    <p><br>
      <span class="little"><?php echo $translate['licencia_cc_imagenes']; ?></span><br>
