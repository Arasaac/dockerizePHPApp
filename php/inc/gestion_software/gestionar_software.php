...
$row=$query->datos_software($_GET['id']);
$autores=$query->listar_autores_sin($row['software_autor']);
$tm=$query->listar_tipo_software();
$so=$query->listar_so_software();
$boton_enviar='Modificar software';
$web_envio='modificar_ficha_software.php';
$id_software=$_GET['id'];
...
$tm=$query->listar_tipo_software();
$so=$query->listar_so_software();
$boton_enviar='Añadir Ficha Software';
$web_envio='add_ficha_software.php';
$id_software='';
}
?>
...
function toggleAreas() {
	...
	area2=new nicEditor({iconsPath : '../../js/nicEdit/nicEditorIcons.gif'}).panelInstance('objetivo');
	area3=new nicEditor({iconsPath : '../../js/nicEdit/nicEditorIcons.gif'}).panelInstance('informacion_adicional');
}

function removeAreas() {
	area1.removeInstance('descripcion');
	area2.removeInstance('objetivo');
	area3.removeInstance('informacion_adicional');
}
...
<script language="javascript" type="text/javascript">
	...
	function customSave() {
		...
		cargar_div('<?php echo $web_envio ?>','descripcion='+document.getElementById('descripcion').value+'&titulo='+document.getElementById('titulo').value+'&objetivo='+document.getElementById('objetivo').value+'&informacion_adicional='+document.getElementById('informacion_adicional').value+'&estado='+document.getElementById('estado').value+'&destacado='+document.getElementById('destacado').checked+'&PickArchivos='+document.getElementById('archivos').value+'&PickCapturas='+document.getElementById('capturas').value+'&Pickso='+document.getElementById('so').value+'&PickAutores='+document.getElementById('autores').value+'&PickTipo='+document.getElementById('tipo').value+'&id_licencia='+document.getElementById('licencia').value+'&PickIdiomas='+document.getElementById('idiomas').value+'&url1='+document.getElementById('url1').value+'&url2='+document.getElementById('url2').value+'&url3='+document.getElementById('url3').value+'&id_software='+document.getElementById('id_software').value+'&precio='+document.getElementById('precio').value+'','mensaje_actualizacion');
...
function selIt_sin() {
  ...
  var pickList3 = document.getElementById("Pickso[]");
  var pickOptions3 = pickList3.options;
  var pickOLength3 = pickOptions3.length;
  for (var c = 0; c < pickOLength3; c++) {
    document.getElementById('so').value = document.getElementById('so').value + '{'+pickOptions3[c].value+'}';
    ...
    <tr>
      <td width="38%" valign="top"><p><strong>Estado:</strong>
          <select name="estado" id="estado">
            <?php if (isset($row['software_estado'])) { ?>
            <option value="2" <?php if ($row['software_estado']==2) { echo "selected"; } ?>>Pendiente revisi&oacute;n</option>
            <option value="1" <?php if ($row['software_estado']==1) { echo "selected"; } ?>>Visible</option>
            <option value="0 <?php if ($row['software_estado']==0) { echo "selected"; } ?>">No Visible</option>
            <?php } else { ?>
        ...
        <p><strong>Destacado:</strong>
          <input name="destacado" type="checkbox" id="destacado" value="1" <?php if ($row['software_destacado']==1) { echo ' checked="checked"'; } ?> />
          <label for="destacado"></label>
        </p>
        <p><strong>T&iacute;tulo:</strong>
          <input name="titulo" type="text" id="titulo" size="25" value="<?php echo $row['software_titulo']; ?>" />
        </p>
        <p align="left"><strong>Descripción:</strong></p>
        <p>
		<textarea name="descripcion" id="descripcion" cols="54" rows="10"><?php echo $row['software_descripcion']; ?></textarea>
        </p>
        <p><strong>Objetivo:</strong></p>
        <p>
        <textarea name="objetivo" id="objetivo" cols="54" rows="10"><?php echo $row['software_objetivo']; ?></textarea>
        </p>
        <p><strong>Información adicional:</strong></p>
        <p>
        <textarea name="informacion_adicional" id="informacion_adicional" cols="54" rows="10"><?php echo $row['software_informacion_adicional']; ?></textarea>
        </p>
        <p></p></td>
      <td width="62%" align="right" valign="top"><table cellpadding="0" cellspacing="0">
        <tr>
          <td colspan="3"><strong>Autores
            ...
            <input name="id_software" type="hidden" id="id_software" value="<?php echo $id_software; ?>" />
          </strong></td>
        </tr>
        <tr>
          ...
          <td><select id="PickAutores[]" style="WIDTH: 250px" multiple="multiple" size="5" name="PickAutores[]">
            <?php 
						if (isset($row['software_autor']) && $row['software_autor'] != '') { 
        ...
        <table cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td><select id="SelectTipo" style="WIDTH: 250px" multiple="multiple" size="5" name="SelectTipo">
              ...
              <option value="<?php echo $row_tm['id_tipo_software'] ?>" sel="sel"><?php echo $row_tm['tipo_software']; ?></option>
              <?php } ?>
            </select></td>
            ...
            <td><select id="PickTipo[]" style="WIDTH: 250px" multiple="multiple" size="5" name="PickTipo[]">
              <?php 
			 if (isset($row['software_tipo']) && $row['software_tipo'] != '') {	
				  ...
				  for ($i=0;$i<count($mt);$i++) { 
					if ($mt[$i]!='') {
					 ...
					 echo '<option value="'.$mt[$i].'" sel="sel">'.$data_tipo['tipo_software'].'</option>';
					}
				  }
			  }
				?>
        ...
        <table cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td><select id="SelectCapturas" style="WIDTH: 250px" multiple="multiple" size="5" name="SelectCapturas">
              <?php 
				$path="../../zona_descargas/software/temp/";
				$dir= opendir ($path);
            ...
            <td><select id="PickCapturas[]" style="WIDTH: 250px" multiple="multiple" size="5" name="PickCapturas[]">
              <?php 
			 if (isset($row['software_capturas']) && $row['software_capturas'] != '') { 
        ...
        <table cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td><select id="PickArchivos[]" style="WIDTH: 250px" multiple="multiple" size="5" name="PickArchivos[]">
              <?php 
			 if (isset($row['software_archivos']) && $row['software_archivos'] != '') { 
        ...
        <table cellpadding="0" cellspacing="0">
          <tr>
            <td colspan="3"><strong>Sistema Operativo
              <input type="hidden" name="so" id="so" />
            </strong></td>
          </tr>
          <tr>
            <td><select id="soList" style="WIDTH: 250px" multiple="multiple" size="5" name="soList">
              <?php while ($row_so=mysql_fetch_array($so)) {  ?>
              <option value="<?php echo $row_so['id_so'] ?>" sel="sel"><?php echo $row_so['so']; ?></option>
              <?php } ?>
            </select></td>
            <td><input onclick="addIt('soList','Pickso[]');" type="button" value="-&gt;" />
              <br />
              <input onclick="delIt('soList','Pickso[]');" type="button" value="&lt;-" /></td>
            <td><select id="Pickso[]" style="WIDTH: 250px" multiple="multiple" size="5" name="Pickso[]">
              <?php 
				if (isset($row['software_so']) && $row['software_so'] != '') {	
				  ...
				  for ($i=0;$i<count($me);$i++) { 
					if ($me[$i]!='') {
					 ...
					 echo '<option value="'.$me[$i].'" sel="sel">'.$data_so['so'].'</option>';
					}
				  }
				}		
				?>
        ...
        <table cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td><select id="PickIdiomas[]" style="WIDTH: 250px" multiple="multiple" size="5" name="PickIdiomas[]">
              <?php 
			 if (isset($row['software_idiomas']) && $row['software_idiomas'] != '') { 
					$id=str_replace('}{',',',$row['software_idiomas']);
				 	$id=str_replace('{','',$id);
				  	$id=str_replace('}','',$id);
				  	$id=explode(',',$id);
        ...
        <p align="left"><strong>Licencia:</strong>
          <select name="licencia" id="licencia" required="1" realname="Categoría" class="fonty">
            <?php if (isset($row['software_licencia'])) { 
		   
				 while ($row_rsLicencia=mysql_fetch_array($licencias)) { 
					?>
            <option value="<?php echo $row_rsLicencia['id_licencia']?>" <?php if ($row['software_licencia']==$row_rsLicencia['id_licencia']) { echo "selected"; } ?>><?php echo utf8_encode($row_rsLicencia['licencia']); ?></option>
            <?php 
					}  
        ...
        <p align="left"><strong>Precio:</strong>
          <input name="precio" type="text" id="precio" size="25" value="<?php echo $row['software_precio']; ?>" />
        </p>
        <p align="left"><strong>URL 1:</strong>
          <input name="url1" type="text" id="url1" size="25" value="<?php echo $row['software_url1']; ?>" />
        </p>
        <p align="left"><strong>URL 2:</strong>
          <input name="url2" type="text" id="url2" size="25" value="<?php echo $row['software_url2']; ?>" />
        </p>
        <p align="left"><strong>URL 3:</strong>
          <input name="url3" type="text" id="url3" size="25" value="<?php echo $row['software_url3']; ?>" />
        </p></td>
    </tr>
  </table>
