...
for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) { // NUMERO DE FILAS 
	...
	if ($data->sheets[0]['cells'][$i][1] != "" && $data->sheets[0]['cells'][$i][2] != "") {
	  		...
	  		$sql = $query->add_traduccion_temp($data->sheets[0]['cells'][$i][1], $id_idioma, $data->sheets[0]['cells'][$i][2],$data->sheets[0]['cells'][$i][3],'',1,1);	
			//id_palabra,id_idioma,traduccion,explicacion,pronunciacion,id_colaborador,estado
			
	} else {  $error1=true; echo $i.'/'.$data->sheets[0]['cells'][$i][1].'/'.$data->sheets[0]['cells'][$i][2].'<br />'; } // Cierro el IF de comprobación del array si está en blanco
