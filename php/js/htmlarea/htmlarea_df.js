...
HTMLArea.Config = function () {
	...
	this.btnList = {
		bold: [ "Bold", "ed_format_boldg.gif", false, function(e) {e.execCommand("bold");} ],
		italic: [ "Italic", "ed_format_italicg.gif", false, function(e) {e.execCommand("italic");} ],
		...
		forecolor: [ "Font Color", "ed_color_fgg.gif", false, function(e) {e.execCommand("forecolor");} ],
		...
		if (options) {
			...
			HTMLArea._addEvent(el, "focus", function () {
				if (obj.enabled) {
					switch (txt) {
						case "fontsize":
						playsound('../../repositorio/sonidos/tamano_fuente.wav');
						break;
						
						case "fontname":
						playsound('../../repositorio/sonidos/tipo_fuente.wav');
						break;
						
						case "formatblock":
						playsound('../../repositorio/sonidos/formato_fuente.wav');
						break;
					}
				}
			});
		...
		if (!el && btn) {
			...
			HTMLArea._addEvent(el, "focus", function () {
				if (obj.enabled) {
					...
					switch (txt) {
						case "bold":
						playsound('../../repositorio/sonidos/negrita.wav');
						break;
						
						case "italic":
						playsound('../../repositorio/sonidos/cursiva.wav');
						break;
						
						case "forecolor":
						playsound('../../repositorio/sonidos/color_texto.wav');
						break;
					}
			...
			img.src = btn[1];
			img.style.width = _editor_iconos+"px";
			img.style.height = _editor_iconos+"px";
			el.appendChild(img);
		} else if (!el) {
			el = createSelect(txt);
		}
	...
	function initIframe() {
		...
		if (!editor.config.fullPage) {
			...
			html += "<style>" + editor.config.pageStyle +
				" html,body { border: 0px;  font-size: "+ _editor_font_size +"%; font-family: Arial; }</style>\n";
			html += "</head>\n";
			html += "<body>\n";
		...
		// MODIFICADO POR MI PARA EVITAR QUE AL INICIAR LA PÁGINA EL FOCUS SE SITUE SOBRE EL CAMPO DE TEXTO Y SE SITÚE EN EL DE TÍTULO
		//***************************************************************************************************************************//
		
		//editor.focusEditor();
		
		// DESCOMENTAR LA LINEA SUPERIOR PARA VOLVER AL ESTADO ORIGINAL
		//***************************************************************************************************************************//
		
		// intercept some events; for updating the toolbar & keyboard handlers
		
		/** He añadido "focus" a la lista para conseguir que al poner el focus sobre el area de
		texto pueda sonar un sonido predeterminado que facilite la accesibilidad a personas ciegas*/
		//***************************************************************************************************************************//
		
		HTMLArea._addEvents
			(doc, ["keydown", "keypress", "mousedown", "mouseup", "drag", "focus"],
			 function (event) {
...
HTMLArea.loadStyle("htmlarea_df.css");
...
HTMLArea.prototype.execCommand = function(cmdID, UI, param) {
	...
	switch (cmdID) {
	    ...
	    case "forecolor":
		this._popupDialog("select_color_i.html", function(color) {
...
HTMLArea.prototype._editorEvent = function(ev) {
	...
	/** He añadido ésto para conseguir que al poner el focus sobre el area de
	texto pueda sonar un sonido predeterminado que facilite la accesibilidad a personas ciegas*/
	var accion = (HTMLArea.is_ie && ev.type == "focus");
	
	if (!accion) {
		playsound('../../repositorio/sonidos/texto.wav');
	}
	/*    Hasta aqui    */
