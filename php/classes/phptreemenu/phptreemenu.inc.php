	...
	)
{
	...
	while (list($key, $value) = each($_GET)) {
		...
		if ($key != 'p' && $value != '' && $key != 'subcategoria' && $value != 'l') {
			$query .= '&amp;' . $key . '=' . $value;
		} elseif ($key == 'subcategoria') {
			for($t = 0; $t < sizeof($value); ++$t) {
				$query .= '&amp;subcategoria[]='.$value[$t];
			}
	...
	for ($cnt=$this->_firstItem[$menu_name]; $cnt<=$this->_lastItem[$menu_name]; $cnt++) {
		...
		if ($visible[$cnt]) {
			...
			for ($i=0; $i<$this->tree[$cnt]['level']-1; $i++) {
				...
				$this->_phpTreeMenu[$menu_name] .= '<img class="imgs" src="' . $img . '" alt="' . $alt . '" />';
			}
			...
			// corner at end of subtree or t-split
				if ($not_a_leaf) {
					...
					$this->_phpTreeMenu[$menu_name] .= '<a name="' . $cnt . '" class="' . $linkstyle . '" href="' . $url . $query . $params . '#' . $cnt . '"><img class="imgs" src="' . $img . '" alt="' . $alt . '" /></a>';
				} else {
					$this->_phpTreeMenu[$menu_name] .= '<img class="imgs" src="' . $img_corner . '" alt="' . $alt_corner . '" />';
				}
			...
			if ($this->tree[$cnt]['parsed_href'] == '' || $this->tree[$cnt]['parsed_href'] == '#') {
				...
				$a_href_close_img = '</a>';
				$chequeado='';
				if ($this->tree[$cnt]['icon']=='activar') { $chequeado='checked="checked'; } else { $chequeado=''; }
				
				if ($not_a_leaf ==1) { 
				$a_href_open = '<input name="categoria[]" type="checkbox" value="'.$this->tree[$cnt]['title'].'" '.$chequeado.'/><a href="' . $this->tree[$cnt]['parsed_href'] . '"' . $this->tree[$cnt]['parsed_title'] . $this->tree[$cnt]['parsed_target'] . ' class="' . $linkstyle . '">';
				$a_href_close = '</a>';
				} else { 
				$a_href_open = '<input name="subcategoria[]" type="checkbox" value="'.$this->tree[$cnt]['title'].'" '.$chequeado.'/><a href="' . $this->tree[$cnt]['parsed_href'] . '"' . $this->tree[$cnt]['parsed_title'] . $this->tree[$cnt]['parsed_target'] . ' class="' . $linkstyle . '">';
				$a_href_close = '</a>';
				}
			}
			...
			if ($not_a_leaf) {
				...
				$this->_phpTreeMenu[$menu_name] .= $a_href_open_img . '<img class="imgs" src="' . $img . '" alt="' . $alt . '" />' . $a_href_close_img;
			} else {
				if ($this->tree[$cnt]['parsed_icon'] != '') {
					$this->_phpTreeMenu[$menu_name] .= $a_href_open_img . '<img src="' . $this->tree[$cnt]['parsed_icon'] . '" width="' . $this->tree[$cnt]['iconwidth'] . '" height="' . $this->tree[$cnt]['iconheight'] . '" alt="' . $alt_leaf . '" />' . $a_href_close_img;
				} else {
					$this->_phpTreeMenu[$menu_name] .= $a_href_open_img . '<img class="imgs" src="' . $img_leaf . '" alt="' . $alt_leaf . '" />' . $a_href_close_img;
				}
			}
