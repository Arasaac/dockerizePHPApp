...
function setMenuStructureFile($tree_file)
{
	...
	while ($buffer = fgets($fd, 4096)) {
		$buffer = preg_replace('/'.chr(13).'/i', '', $buffer);	// Microsoft Stupidity Suppression
...
{
	$this->menuStructure = preg_replace('/'.chr(13).'/i', '', $tree_string);	// Microsoft Stupidity Suppression
	if ($this->menuStructure == '') {
