	...
	$lse_byn=0;
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
					...
					if($query) {
						...
						// While there are results loop through them - fetching an Object (i like PHP5 btw!).
							while ($row=mysql_fetch_array($query)) {
								...
								echo '<b>'.CheckSentence($row['palabra'],$queryString, "color:#61ade7;").': '.substr(utf8_encode($row['definicion']), 0, 100).'...</b></li>';
				...
				} elseif ($_POST['language']!='es') { //Cierro el If de comprobación de si es en castellano
