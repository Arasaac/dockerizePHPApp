<?php
// $Header: /cvsroot/html2ps/box.generic.php,v 1.48 2005/10/26 15:13:19 Konstantin Exp $
...
class GenericBoxPDF {
  ...
  function GenericBoxPDF() {
    ...
    $this->cellpadding = units2pt($handler->get());

    // '-cellspacing'
    $handler =& get_css_handler('-cellspacing');
    $this->cellspacing = units2pt($handler->get());
    ...
    // 'font-size'
    $this->font_size = units2pt(get_font_size());
    ...
    $this->text_indent->units2pt($this);
    ...
    $value = $handler->get();
    $this->left = $value === null ? $this->get_left() : $value;
  ...
  function _calc_percentage_margins(&$parent) {
    if ($this->margin->left->percentage !== null) {
      $this->margin->left->value = $parent->get_width() * $this->margin->left->percentage / 100;
    };

    if ($this->margin->right->percentage !== null) {
  ...
  function _calc_percentage_padding(&$parent) {
    if ($this->padding->left->percentage !== null) {
      $this->padding->left->value = $parent->get_width() * $this->padding->left->percentage / 100;
    };

    if ($this->padding->right->percentage !== null) {
  ...
  function _calc_percentage_width(&$parent, &$context) {
    if (is_a($this->_width_constraint, "WCFraction")) {
      ...
      $width = $this->_width_constraint->apply($this->width, $parent->get_width());
...
//     };
  ...
  function _get_containing_block() {
    ...
    if ($parent === null) { die("No containing block found for absolute-positioned element"); };
    ...
    while (($parent->parent !== null) && ($parent->position === POSITION_STATIC)) { $parent =& $parent->parent; }
  ...
  function _apply_line_height() {
    ...
    $new_height = $this->line_height->apply($height);
    $under      = $height - $this->default_baseline;

    if ($height > 0) {
      $scale = $this->line_height->apply($height) / $height;
    } else {
      $scale = 0;
    };

    $this->put_height($new_height);
    $this->default_baseline = $new_height - $under*$scale;
  }

//   function _apply_vertical_align(&$parent) {
//     // Now vertical align is applied in close_line
//     // $this->vertical_align->apply($this,$parent);
//   }
  ...
  function get_right() { return $this->get_left() + $this->get_width(); }

  function get_left() { return $this->_left; }
  ...
  function to_ps_css(&$psdata) {
    ...
    // CSS positioning properties
    // 'left'
    if ($this->left !== CSSLeft::default_value()) {
      $psdata->write("dup /left ".CSSLeft::value2ps($this->left)." put-css-value\n");
    };
