<?php
// $Header: /cvsroot/html2ps/css.selectors.inc.php,v 1.11 2005/09/25 16:21:44 Konstantin Exp $
...
function node_have_class($root, $target_class) {
  // We do not need to check if $root has this attribute, as get_attribute returns 
  // empty string if the attribute is absent
  //
  $classes = preg_split("/\s+/", strtolower($root->get_attribute('class')));

  foreach ($classes as $class) {
    if ($class === $target_class) { return true; };
  }
