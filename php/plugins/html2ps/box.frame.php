<?php
// $Header: /cvsroot/html2ps/box.frame.php,v 1.11 2005/09/25 16:21:43 Konstantin Exp $

class FrameBoxPDF extends GenericContainerBoxPDF {
  function &create($viewport, &$root) {
    return new FrameBoxPDF($viewport, $root);
  }
  ...
  function reflow(&$parent, &$context) {
    ...
    $hc = new HCConstraint($this->get_height(), $this->get_height(), $this->get_height());
  ...
  function FrameBoxPDF($viewport, &$root) {
    ...
    $this->GenericContainerBoxPDF($viewport,$root);
...
class FramesetBoxPDF extends GenericContainerBoxPDF {
  var $rows;
  var $cols;

  function &create($viewport, &$root) {
    return new FramesetBoxPDF($viewport, $root);
  }

  function FramesetBoxPDF($viewport, &$root) {
    $this->GenericContainerBoxPDF($viewport,$root);
    $this->create_content($viewport, $root);
  ...
  function reflow(&$parent, &$context) {
    ...
    for ($i=0; $i < count($this->content); $i++) {
      ...
      $context->push_viewport(new FlowViewport($frame));
