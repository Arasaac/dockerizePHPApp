  ...
  function float_left_x($x, $y) {
    ...
    for ($i=0; $i<count($floats); $i++) {
      ...
      if ($float->float == FLOAT_LEFT) {
        ...
        //       print($float->get_top_margin()."<br>");
        //       print($float->get_bottom_margin()."<br>");
        //       print($y);
      

        // To fix the rounding errors when doing the equility comparison
        $eps = 0.001;

        if ($float->get_top_margin() + $eps >= $y &&
...
//        print("Y=".$y."<br>");
      ...
      $eps = 0.001;
      if ($parent->get_right() + $eps >= $x + $width ||
...
//       print("Parent WIDTH:".$parent->get_width()."<br>");
//       print("Child WIDTH:".$width."<br>");
