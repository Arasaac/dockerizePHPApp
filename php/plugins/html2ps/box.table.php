<?php
// $Header: /cvsroot/html2ps/box.table.php,v 1.36 2005/10/06 03:55:16 Konstantin Exp $
...
class TableBoxPDF extends GenericContainerBoxPDF {
  var $cwc;
...
//   }
  ...
  function get_width() {
//     if ($this->parent) {
      if (!is_a($this->parent, "TableCellBoxPDF") || 
...
//       $maxwc[$i] = max($minw[$i], $cwc->apply($maxw[$i], $this_width));
    ...
    if (array_sum($widths) < $width) {
      ...
      $widths = expand_to_with_flags($width, 
                                     $widths,
                                     $this->get_non_constrained_width_flags());
    };
